...
class Screen_EPD_EXT3 final : public hV_Screen_Buffer
{
  public:
    ...
    uint8_t * blackimg () {return _newImage ; }
    uint8_t * redimg() { return _newImage+_pageColourSize ; }
  ...
  protected:
    ...
    ///
    void _sendIndexData(uint8_t index, uint8_t * data, uint32_t size);
    ...
    ///
    void _sendIndexDataMaster(uint8_t index, uint8_t * data, uint32_t size);
    ...
    /// @note Valid only for 9.7 and 12.20" screens
    ///
    void _sendIndexDataSlave(uint8_t index, uint8_t * data, uint32_t size);
