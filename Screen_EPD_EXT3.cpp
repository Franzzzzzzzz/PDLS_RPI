...
void Screen_EPD_EXT3::begin()
{
    ...
    SPIio = new SPI("/dev/spidev0.0", &spi_config);
    bool res = SPIio->begin() ;
    if (!res) {printf("ERROR cannot open SPI\n") ; std::exit(1);}
#endif
...
// Utilities
void Screen_EPD_EXT3::_sendIndexData(uint8_t index, uint8_t * data, uint32_t size)
{
...
void Screen_EPD_EXT3::_sendIndexDataMaster(uint8_t index, uint8_t * data, uint32_t size)
{
    if (_pin.panelCSS != NOT_CONNECTED)
    {
...
void Screen_EPD_EXT3::_sendIndexDataSlave(uint8_t index, uint8_t * data, uint32_t size)
{
